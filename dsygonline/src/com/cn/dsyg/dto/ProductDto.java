			...
			if(item13 != null && !"".equals(item13) && item12 != null && !"".equals(item12)) {
				...
				for(int i = 1; i < list.length; i++) {
					if(!"".equals(list[i])) {
						...
						BigDecimal price = basetaxprice.multiply(new BigDecimal(startll[1]).divide(new BigDecimal(100))).setScale(3, BigDecimal.ROUND_HALF_UP);
						...
						if(i == list.length - 1) {
							//最后一条记录
							BigDecimal lastprice = basetaxprice.multiply(new BigDecimal(ll[1]).divide(new BigDecimal(100))).setScale(3, BigDecimal.ROUND_HALF_UP);
	...
	public BigDecimal getOnlineprice() {
		...
		if(basetaxprice != null) {
			if(item13 != null && !"".equals(item13)) {
				...
				onlineprice = basetaxprice.multiply(new BigDecimal(startll[1]).divide(new BigDecimal(100))).setScale(3, BigDecimal.ROUND_HALF_UP);
			}
		}
		return onlineprice;
	}
